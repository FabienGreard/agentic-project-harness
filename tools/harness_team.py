...
CATALOG_NAME = "tools/team-presets.json"
TEAM_NAME = "docs/state/team.json"
...
Read AGENTS.md, every applicable .agents/rules/ file, docs/state/team.json, docs/roles/contractor.md, and the complete assignment before acting.
...
""",
        ...
        "internal-audit.toml": role_config(
            name="internal_audit",
            description="Hidden disposable read-only harness evaluator; not a project team member or QA Consultant.",
            reasoning=reasoning["internalAudit"],
            read_only=True,
            instructions="""
You are Internal Audit, a disposable independent evaluator of the harness rather than a member of the project team.
Read AGENTS.md, applicable .agents/rules/ files, docs/state/team.json, docs/roles/internal-audit.md, and the exact evaluation contract.
...
def validate_team(team: dict[str, Any], catalog: dict[str, Any]) -> list[str]:
    ...
    if set(team) != expected:
        errors.append("docs/state/team.json does not match the team record contract")
        return errors
    if type(team.get("schemaVersion")) is not int or team.get("schemaVersion") != 1 or team.get("recordType") != "team":
        errors.append("docs/state/team.json has an unsupported schema")
    try:
    ...
    if team.get("presetLabel") != preset["label"]:
        errors.append("docs/state/team.json preset label drift")
    if team.get("commonNames") != catalog["commonNames"]:
        errors.append("docs/state/team.json common-name drift")
    for role in ("management", "operations"):
        current = team.get(role)
        definition = preset[role]
        if not isinstance(current, dict) or current.get("title") != definition["title"]:
            errors.append(f"docs/state/team.json {role} persona drift")
    if team.get("contractorBench") != preset["contractorBench"]:
        errors.append("docs/state/team.json Contractor bench drift")
    if team.get("internalAudit") != catalog["internalAudit"]:
        errors.append("docs/state/team.json Internal Audit drift")
    consultants = team.get("consultants")
    if not isinstance(consultants, list):
        errors.append("docs/state/team.json Consultants must be an array")
        return errors
    ids: set[str] = set()
    titles: set[str] = set()
    for index, raw in enumerate(consultants):
        if not isinstance(raw, dict):
            errors.append(f"docs/state/team.json consultant {index} must be an object")
            continue
        try:
            ...
            errors.append(f"docs/state/team.json consultant {index}: {error}")
            continue
        identifier = normalized["id"]
        title = normalized["title"].casefold()
        if raw.get("source") not in {"preset", "custom"}:
            errors.append(
                f"docs/state/team.json Consultant {identifier} has invalid source"
            )
        if identifier in ids or title in titles:
            errors.append("docs/state/team.json has duplicate Consultant identities")
        ids.add(identifier)
        titles.add(title)
        if raw.get("nonAuthorities") != catalog["consultantNonAuthorities"]:
            errors.append(f"docs/state/team.json Consultant {identifier} has authority drift")
        if raw.get("status") not in {"active", "inactive"}:
            errors.append(f"docs/state/team.json Consultant {identifier} has invalid status")
        expected_path = f"{AGENTS_DIR}/consultant-{identifier}.toml"
        if raw.get("configPath") != expected_path:
            errors.append(f"docs/state/team.json Consultant {identifier} has invalid configPath")
        ...
        if raw.get("status") == "active" and not (
            ...
            errors.append(f"docs/state/team.json Consultant {identifier} lacks a config baseline")
        for field in ("hiredAt", "firedAt", "manualAction"):
            if not isinstance(raw.get(field), str):
                errors.append(f"docs/state/team.json Consultant {identifier} has invalid {field}")
        if not isinstance(raw.get("preservedConfig"), bool):
            errors.append(f"docs/state/team.json Consultant {identifier} has invalid preservedConfig")
    if team.get("reasoning") != reasoning:
        errors.append("docs/state/team.json reasoning is not normalized")
    return errors
...
def transaction_directory(root: Path, transaction_id: str) -> Path:
    ...
    transaction = (
        state_home / "agentic-project-harness" / project_id / "team" / transaction_id
...
def load_dashboard_module(root: Path):
    path = inside(root, "tools/harness_state.py")
...
def metadata_with_team_change(
    *, metadata: dict[str, Any], add: dict[str, str] | None = None, remove: str | None = None,
    ...
    if not isinstance(managed, dict):
        raise TeamError("installed harness metadata has no managed-file map")
    if add:
        managed[add["path"]] = {
    ...
    if preserve:
        managed[preserve["path"]] = {
            "ownership": "project-owned",
            "baselineSha256": preserve["sha256"],
        }
    if refresh:
...
def hire_consultant(
    *, project_root: Path, consultant_id: str | None, custom_path: Path | None
    ...
    root = project_root.resolve()
    catalog = load_catalog(root)
    team = read_json(inside(root, TEAM_NAME))
    metadata = read_json(inside(root, METADATA_NAME))
    ...
    report = {
        ...
        "writes": [config_path, TEAM_NAME, "docs/index.html", METADATA_NAME],
        "removes": [],
        "manualActions": [],
    }
    transaction = transaction_write(
        ...
        writes={
            ...
            "docs/index.html": dashboard.encode("utf-8"),
...
def fire_consultant(*, project_root: Path, consultant_id: str) -> dict[str, Any]:
    ...
    report = {
        ...
        "writes": [TEAM_NAME, "docs/index.html", METADATA_NAME],
        "removes": removes,
    ...
    try:
        ...
        if args.command == "check":
            ...
            if metadata.get("projectType") != team.get("preset"):
                errors.append("harness metadata projectType differs from team preset")
            try:
                metadata_reasoning = normalized_reasoning(metadata.get("reasoning"))
            except TeamError as error:
                errors.append(f"harness metadata reasoning is invalid: {error}")
            else:
                if metadata_reasoning != team.get("reasoning"):
                    errors.append("harness metadata reasoning differs from team reasoning")
            if not errors:
                configs = render_team_configs(team=team, catalog=catalog)
                allowed_preserved = {
                ...
                if metadata.get("installationStatus") != "Template":
                    managed = metadata.get("managedFiles")
                    if not isinstance(managed, dict):
                        errors.append("installed harness metadata lacks managedFiles")
                    else:
                        ...
                        for consultant in team["consultants"]:
                            if consultant["status"] != "inactive" or not consultant["preservedConfig"]:
                                continue
                            record = managed.get(consultant["configPath"])
                            if not isinstance(record, dict) or record.get("ownership") != "project-owned":
                                errors.append(
