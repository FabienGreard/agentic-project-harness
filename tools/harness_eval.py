    ...
    def permanent_task_lifecycle(self) -> None:
        ...
        shared_contracts = {
            "rule": (
                ...
                "named return trigger",
                "pauses when no meaningful action remains",
                "poll delegated work",
            ),
            "management_config": (
                "permanent top-level task",
                "event-driven run-to-idle lifecycle",
            ),
            "operations_config": (
                "permanent top-level task",
                "event-driven run-to-idle lifecycle",
            ...
            "management": (
                "permanent top-level task",
                "owner/action/return trigger",
                "pause without polling",
            ),
            "operations": (
                "permanent top-level task",
                "owner/action/return trigger",
            ...
            "workflow": (
                ...
                "event-driven run-to-idle lifecycles",
                "owner/action/return trigger",
                "pauses without polling",
            ),
            "registry": (
                "event-driven run-to-idle",
                "owner/action/return trigger",
                "pause without polling",
            ),
            "guide": (
        ...
        self.record(
            ...
            "; ".join(failures)
            if failures
            else "Management, Operations, and active Consultants are event-driven, run-to-idle, and covered by explicit handoff contracts",
        )
