...
Baton is being separated into a normal product/source repository and an isolated consumer control plane. The source repository owns product code, tests, release tooling, documentation, and its own live `.baton/` state. Consumer payloads are built only from `packages/consumer/`.
