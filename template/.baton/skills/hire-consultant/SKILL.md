...
description: Hire a curated or schema-valid custom Consultant into an Baton team. Use when the user explicitly asks to hire, add, recruit, or onboard a recurring expert role, or invokes $hire-consultant. Do not use for disposable execution work, which Operations assigns to Contractors.
---

# Hire a Consultant

Use the installed deterministic team engine. Do not hand-edit agent configs or `.baton/state/team.json`.
...
4. Inspect the current preset catalog with `.baton/bin/baton _team catalog --preset <preset> --field consultants --json`.
...
```sh
.baton/bin/baton _team hire --project-root . --consultant <id> --yes --json
```

For a custom Consultant, prepare a temporary JSON object matching `.baton/schemas/consultant.schema.json`, then run:

```sh
.baton/bin/baton _team hire --project-root . --custom <temporary-json-path> --yes --json
```
...
Run `.baton/bin/baton _team check --project-root . --json` and `.baton/bin/baton _state check --json`. Report the hired title, domain, config path, transaction/report/backup paths, and task-message wake requirement. Do not start or wake the new Consultant task unless the user separately requests it.
