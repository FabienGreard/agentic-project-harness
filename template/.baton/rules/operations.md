...
Public skill discovery is all-or-nothing: an occupied short skill name that is not Baton's exact link blocks installation. The only legacy exception is a complete unchanged v0.5 discovery tree authenticated against its stable origin and per-file baselines: adoption may quarantine it in `Needs Integration` only, create zero v0.6 links, preserve every legacy path, and block activation until human-approved cleanup replaces the whole discovery set with every exact link. `scrap` requires an immutable reviewed plan, exact current-tree verification, an external backup, and rollback; it never deletes that backup or a preserved collision.
