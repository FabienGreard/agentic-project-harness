#!/usr/bin/env python3
"""Validate and transactionally update Baton's canonical operational state."""
...
def read_json(path: Path, errors: list[str]) -> dict[str, Any] | None:
    if path.is_symlink():
        errors.append(f"{path.relative_to(ROOT)}: symbolic-link JSON records are unsafe")
        return None
    try:
...
def validate_schema_files(errors: list[str]) -> None:
    for name in (*RECORD_NAMES, "operation", "consultant"):
        ...
        if name != "consultant" and (
            ...
            errors.append(f".baton/schemas/{name}.schema.json: missing schemaVersion requirement")
        if schema.get("type") != "object":
            errors.append(f".baton/schemas/{name}.schema.json: expected object schema")


def validate_record(name: str, data: dict[str, Any], errors: list[str]) -> None:
    prefix = f".baton/state/{name}.json"
    if not isinstance(data, dict):
    ...
    try:
        ...
        errors.append(f".baton/schemas/{name}.schema.json: {error}")
        return
...
def validate_consistency(records: dict[str, dict[str, Any]], errors: list[str]) -> None:
    ...
    if duplicate_goals:
        errors.append(
            ".baton/state/goals.json: duplicate goal ids: "
    ...
    if current_goal and current_goal not in known_goals:
        errors.append(f".baton/state/project.json: unknown currentGoal {current_goal!r}")
    elif current_goal and goal_map.get(current_goal, {}).get("status") == "Done":
        errors.append(".baton/state/project.json: currentGoal cannot be Done")
    active_goal_ids = [
        goal.get("id")
        for goal in goals
    ...
    if len(active_goal_ids) > 1:
        errors.append(
            ".baton/state/goals.json: only one goal may be Active or Review"
        )
    if active_goal_ids and active_goal_ids[0] != current_goal:
        errors.append(
            ".baton/state/project.json: currentGoal must identify the Active or Review goal"
        )
    for goal in goals:
        ...
        if unknown:
            errors.append(
                f".baton/state/goals.json: {goal.get('id')} has unknown dependencies: "
                + ", ".join(sorted(unknown))
            )
        if goal.get("id") in dependencies:
            errors.append(f".baton/state/goals.json: {goal.get('id')} depends on itself")
        if goal.get("status") in {"Ready", "Active", "Review", "Done"}:
            ...
            if incomplete:
                errors.append(
                    f".baton/state/goals.json: {goal.get('status')} goal {goal.get('id')} has incomplete dependencies: "
                    + ", ".join(sorted(incomplete))
                )
    cycle = dependency_cycle(goals)
    if cycle:
        errors.append(
            ".baton/state/goals.json: dependency cycle: " + " -> ".join(cycle)
        )
    ...
    for (ticket_id, consultant_id, stage), grouped in consultant_reviews_by_key.items():
        if len(grouped) > 1:
            errors.append(
                ".baton/state/reviews.json: multiple Consultant review decisions for "
    ...
    for goal in goals:
        ...
        if active_linked:
            errors.append(
                f".baton/state/goals.json: Done goal {goal.get('id')} has non-terminal tickets: "
                + ", ".join(sorted(active_linked))
            )
        if owned_linked:
            errors.append(
                f".baton/state/goals.json: Done goal {goal.get('id')} retains active ownership: "
                + ", ".join(sorted(owned_linked))
            )
        if goal.get("blockers") or blocked_linked:
            errors.append(
                f".baton/state/goals.json: Done goal {goal.get('id')} retains blockers or open decisions"
            )
    for ticket in tickets:
        ...
        if ticket.get("goal") and ticket.get("goal") not in known_goals:
            errors.append(
                f".baton/state/tickets.json: {ticket.get('id')} has unknown goal {ticket.get('goal')!r}"
            )
        required_consultants = ticket.get("requiredConsultantIds", [])
        if isinstance(required_consultants, list):
            ...
            if unknown_consultants:
                errors.append(
                    f".baton/state/tickets.json: {ticket.get('id')} requires unknown Consultants: "
                ...
                errors.append(
                    f".baton/state/tickets.json: {ticket.get('id')} requires inactive Consultants: "
            ...
            if ticket.get("status") in EXECUTABLE_STATUS:
                ...
                if missing_readiness:
                    errors.append(
                        f".baton/state/tickets.json: executable ticket {ticket.get('id')} lacks approved Consultant readiness for: "
            ...
            if ticket.get("status") == "Done":
                ...
                if missing_acceptance:
                    errors.append(
                        f".baton/state/tickets.json: Done ticket {ticket.get('id')} lacks approved Consultant acceptance for: "
        ...
        if unknown:
            errors.append(f".baton/state/tickets.json: {ticket.get('id')} has unknown dependencies: {', '.join(sorted(unknown))}")
        if ticket.get("id") in dependencies:
            errors.append(f".baton/state/tickets.json: {ticket.get('id')} depends on itself")
        if ticket.get("status") == "Ready":
            ...
            if incomplete:
                errors.append(
                    f".baton/state/tickets.json: Ready ticket {ticket.get('id')} has incomplete dependencies: "
                    + ", ".join(sorted(incomplete))
                )
        if ticket.get("status") == "Done":
            ...
            report = repository_path(
                report_path,
                f".baton/state/tickets.json.{ticket.get('id')}.reportPath",
                errors,
            )
            if report is None or report.is_symlink() or not report.is_file():
                errors.append(
                    f".baton/state/tickets.json: Done ticket {ticket.get('id')} requires an existing reportPath"
                )
    ...
    for item in ownership:
        ...
        if item.get("ticket") not in known:
            errors.append(f".baton/state/ownership.json: unknown ticket {item.get('ticket')!r}")
        elif ticket_map[item.get("ticket")].get("status") != OWNERSHIP_TICKET_STATUS.get(item.get("status")):
            errors.append(
                f".baton/state/ownership.json: {item.get('ticket')} status does not match its ticket"
            )
            ...
            for scope in item_scopes:
                ...
                for previous, previous_ticket in scopes:
                    if (
                        ...
                        errors.append(
                            ".baton/state/ownership.json: overlapping scopes: "
    ...
    if duplicate_review_ids:
        errors.append(
            ".baton/state/reviews.json: duplicate review ids: "
    ...
    for item in consultant_reviews:
        ...
        if item.get("ticket") not in known:
            errors.append(
                f".baton/state/reviews.json: Consultant review has unknown ticket {item.get('ticket')!r}"
            )
        if item.get("consultantId") not in consultant_map:
            errors.append(
                f".baton/state/reviews.json: unknown Consultant {item.get('consultantId')!r}"
            )
...
def empty_memory_projection() -> dict[str, Any]:
    return {
        "schemaVersion": 1,
        "revision": 0,
        "bootstrap": {"status": "not-initialized", "roster": []},
        "personnel": [],
        "chronology": [],
        "privacy": "allowlisted; no active memory projection",
    }


def load_memory_projection(errors: list[str]) -> dict[str, Any]:
    memory_path = BATON_DIR / "memory/memory.json"
    history_path = BATON_DIR / "memory/history.jsonl"
    metadata_path = BATON_DIR / "metadata.json"
    expected = False
    if metadata_path.is_file() and not metadata_path.is_symlink():
        metadata = read_json(metadata_path, errors)
        project_owned = metadata.get("projectOwnedFiles", []) if metadata else []
        expected = any(
            isinstance(path, str) and path.startswith(".baton/memory/")
            for path in project_owned
        )
    present = memory_path.is_file() and history_path.is_file()
    if not present:
        if expected:
            errors.append(
                ".baton/memory: active project-local memory is missing; restore it from transactional evidence"
            )
        return empty_memory_projection()
    if memory_path.is_symlink() or history_path.is_symlink():
        errors.append(".baton/memory: symbolic-link memory records are unsafe")
        return empty_memory_projection()
    try:
        from baton_memory import inspect as inspect_memory

        projection = inspect_memory(ROOT, {"section": "projection"})
    except (ImportError, OSError, RuntimeError, ValueError) as error:
        errors.append(f".baton/memory: {error}")
        return empty_memory_projection()
    return projection
...
def render_dashboard(
    ...
    dashboard_records = dict(records)
    dashboard_records["memory"] = memory_projection or empty_memory_projection()
    snapshot = json.dumps(
        dashboard_records, ensure_ascii=False, sort_keys=True, separators=(",", ":")
    ).replace("</", "<\\/")
...
.notion-button:disabled{background:rgba(7,19,28,.55);border-color:var(--line-strong);color:var(--muted);cursor:not-allowed;opacity:.55}
...
.workforce-layout{display:grid;gap:.8rem;grid-template-columns:1fr}.workforce-panel{background:rgba(7,19,28,.76);border:1px solid var(--line-strong);padding:1rem}.workforce-panel h3{color:var(--phosphor);font:700 .78rem/1.2 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.07em;margin:0 0 .9rem;text-transform:uppercase}.workforce-panel h3:before{color:var(--phosphor-muted);content:"[ ";}.workforce-panel h3:after{color:var(--phosphor-muted);content:" ]"}.workforce-bars{display:grid;gap:.72rem;list-style:none;margin:0;padding:0}.workforce-row{align-items:center;border-left:2px solid var(--line-strong);display:grid;gap:.7rem;grid-template-columns:minmax(0,1fr) auto;padding:.2rem 0 .2rem .7rem}.workforce-person{min-width:0}.workforce-person strong{color:var(--phosphor-soft);display:block;font-size:.8rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.workforce-person span{color:var(--muted);display:block;font-size:.7rem;margin-top:.12rem}.workforce-row>b{color:var(--phosphor);font:700 .74rem/1 ui-monospace,SFMono-Regular,Menlo,monospace;text-align:right}.workforce-note{color:var(--muted);font-size:.7rem;margin:.7rem 0 0}
...
@media(max-width:760px){.system-nav{margin-left:-1rem;margin-right:-1rem;overflow:visible;padding-left:1rem;padding-right:1rem}.system-nav a{font-size:.7rem;padding:.85rem .5rem 1rem;white-space:nowrap}.system-nav a:first-child{margin-left:-.5rem}.company-subheading{align-items:start;display:grid;grid-template-columns:1fr}.company-subheading>.count{justify-self:start}.workforce-layout{grid-template-columns:1fr}.workforce-row{grid-template-columns:minmax(0,1fr) auto}.ticket-control-group{display:grid;grid-template-columns:minmax(0,1fr) minmax(150px,.55fr)}.ticket-filter{max-width:none}.pip-hud{grid-template-columns:minmax(0,1fr) 110px}.hud-cell{padding:.55rem .65rem}.hud-phase,.hud-review{display:none}.hud-goal{border-left:1px solid var(--line)}}
...
@media(max-width:480px){.ticket-control-group{grid-template-columns:1fr}}
...
.ticket-search input:disabled,.ticket-filter-trigger:disabled{background:rgba(7,19,28,.55);border-color:var(--line-strong);color:var(--muted);cursor:not-allowed;opacity:.55}.ticket-search:has(input:disabled):before,.ticket-filter-trigger:disabled .ticket-filter-chevron{color:var(--muted);opacity:.55}
...
.company-memory{background:rgba(7,19,28,.48);border:1px solid var(--line-strong);margin-top:1rem;padding:1rem}.memory-layout{display:grid;gap:.8rem;grid-template-columns:minmax(0,1.2fr) minmax(260px,.8fr)}.memory-panel{background:rgba(7,19,28,.76);border:1px solid var(--line-strong);padding:1rem}.memory-panel h3{color:var(--phosphor);font:700 .78rem/1.2 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.07em;margin:0 0 .9rem;text-transform:uppercase}.memory-list,.memory-outcomes{display:grid;gap:.55rem;list-style:none;margin:0;padding:0}.memory-person,.memory-event{border-left:3px solid var(--phosphor-muted);padding:.15rem 0 .15rem .75rem}.memory-person strong,.memory-event strong{color:var(--phosphor-soft);display:block;font-size:.8rem}.memory-person span,.memory-event span{color:var(--muted);display:block;font-size:.7rem;margin-top:.12rem}.memory-person small,.memory-event small{color:#a9c9ef;display:block;font:650 .62rem/1.3 ui-monospace,SFMono-Regular,Menlo,monospace;margin-top:.18rem;text-transform:uppercase}.memory-outcomes{border-top:1px solid var(--line);gap:.4rem;margin-top:.5rem;padding-top:.45rem}.memory-outcome{color:var(--phosphor-soft);font-size:.7rem}.memory-outcome b{color:var(--phosphor);font-size:.62rem;text-transform:uppercase}.memory-source[data-unverified="true"]{color:var(--warning)}.memory-evidence{display:flex;flex-wrap:wrap;gap:.25rem .55rem;margin-top:.22rem}.memory-evidence a{color:#a9c9ef;font:650 .62rem/1.3 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}.memory-empty{color:var(--muted);font-size:.76rem;margin:0}
@media(max-width:760px){.company-subheading{align-items:start;display:grid;grid-template-columns:1fr}.company-subheading>.count{justify-self:start}.memory-layout{grid-template-columns:1fr}}
...
const memoryPerson=(role,seat)=>memoryPeople.find(person=>person.role===role&&identityKey(person.seat)===identityKey(seat)&&!['former','retired','replaced'].includes(person.employmentStatus));
function workforceProfileFor(owner){const explicit=s.workforceProfiles?.[owner];if(explicit)return {owner,...explicit};const key=identityKey(owner),remembered=memoryPeople.find(person=>[person.id,person.name,person.seat].some(value=>identityKey(value)===key));if(remembered)return {owner,name:remembered.name,category:remembered.role,title:remembered.specialty||remembered.seat};const management=team?.management,operations=team?.operations,rememberedManagement=memoryPerson('Management','Management'),rememberedOperations=memoryPerson('Operations','Operations');if(management&&[management.commonName,management.title,'management'].some(value=>identityKey(value)===key))return {owner,name:rememberedManagement?.name||management.commonName||'Management',category:'Management',title:management.title};if(operations&&[operations.commonName,operations.title,'operations'].some(value=>identityKey(value)===key))return {owner,name:rememberedOperations?.name||operations.commonName||'Operations',category:'Operations',title:operations.title};const consultant=activeConsultants.find(item=>[item.id,item.title].some(value=>identityKey(value)===key));if(consultant){const person=memoryPerson('Consultant',consultant.id);return {owner,name:person?.name||consultant.title,category:'Consultant',title:consultant.domain||consultant.headline}}const contractor=(team?.contractorBench||[]).find(item=>identityKey(item.id)===key);if(contractor)return {owner,name:titleCase(contractor.id),category:'Contractor',title:contractor.headline};const dispatched=o.some(item=>identityKey(item.owner)===key);return {owner,name:owner,category:dispatched?'Contractor':'Project team',title:dispatched?'Assigned specialist':'Assigned contributor'}}
...
const configuredIdentity=(role,seat,fallback,title)=>{const person=memoryPerson(role,seat);return {owner:seat,name:person?.name||fallback,category:role,title,taskStatus:person?.taskStatus||'unregistered',total:0,done:0,rate:0}},configuredContractors=memoryPeople.filter(person=>person.role==='Contractor'&&!['former','retired','replaced'].includes(person.employmentStatus)).map(person=>({owner:person.id,name:person.name,category:'Contractor',title:person.specialty||person.seat,taskStatus:person.taskStatus,total:0,done:0,rate:0}));
const assignedPeople=[...workforceByOwner.values()],configuredPeople=[team?.management&&configuredIdentity('Management','Management',team.management.commonName||'Management',team.management.title),team?.operations&&configuredIdentity('Operations','Operations',team.operations.commonName||'Operations',team.operations.title),...activeConsultants.map(item=>configuredIdentity('Consultant',item.id,team?.commonNames?.consultants||item.title,item.domain||item.headline||item.title)),...configuredContractors].filter(Boolean),workforceRoleKey=person=>['Management','Operations'].includes(person.category)?identityKey(person.category):`${identityKey(person.category)}|${identityKey(person.title)}`,assignedRoleKeys=new Set(assignedPeople.map(workforceRoleKey)),unassignedConfiguredPeople=configuredPeople.filter(person=>!assignedRoleKeys.has(workforceRoleKey(person)));const workforceOrder={Management:0,Operations:1,Consultant:2,Contractor:3,'Project team':4},workforcePeople=[...unassignedConfiguredPeople,...assignedPeople].sort((a,b)=>(workforceOrder[a.category]??9)-(workforceOrder[b.category]??9)||a.name.localeCompare(b.name)),workforceTotal=assignedPeople.reduce((sum,person)=>sum+person.total,0);
...
def command_check(as_json: bool) -> int:
    ...
    memory_projection = load_memory_projection(errors)
    if not errors and (
        not DASHBOARD.is_file()
        or DASHBOARD.read_text(encoding="utf-8")
...
def command_apply(operation_path: Path, as_json: bool) -> int:
    ...
    try:
        ...
        errors.append(f".baton/schemas/operation.schema.json: {error}")
    else:
    ...
    if errors:
        emit(result(False, errors), as_json)
        return 1
    dashboard = render_dashboard(proposed, memory_projection)
    files[DASHBOARD] = dashboard
    metadata_path = ROOT / ".baton/metadata.json"
