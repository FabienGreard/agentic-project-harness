...
1. Load the canonical operational JSON and validate it with `python3 tools/harness_state.py check` before relying on it.
...
3. For installation, adoption, update, migration, or rollback, inspect local status and the recorded `.agent-harness.json` provenance before proposing a change.
...
- Use one `install.sh` lifecycle with its smart default, `status`, and `update` commands.
- Install and update only from approved stable GitHub release assets and immutable recorded provenance.
- Keep updates atomic: re-derive the origin baseline from its full commit and pinned schema-v2 manifest digest before planning from that verified baseline, current local state, and target release; preserve external transaction backups and offer documented rollback recovery.
...
- Record installation version, stable provenance, ownership classes, checksums, status, and migrations in `.agent-harness.json`.
...
Validation:
The state tool validates canonical JSON, the generated dashboard reflects that state, `.agent-harness.json` supplies verifiable provenance and recovery evidence, and every consequential action remains at its authorized human or role boundary.
...
- This rule defines the v0.5 operational contract. Availability of a command, migration, generated dashboard, or release asset still requires its own implementation and verification evidence.
